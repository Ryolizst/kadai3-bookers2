...
describe '[STEP2] ユーザログイン後のテスト' do
  ...
  visit books_path
  fill_in 'book[title]', with: 'Sample Title'   # タイトルが空でないか確認
  fill_in 'book[body]', with: 'This is a sample book body'  # ボディが空でないか確認
  expect { click_button 'Create Book' }.to change(user.books, :count).by(1)  # 1増えることを確認
      end
    end
